...
Ff you make changes in datamodel.graphql file deploy app and what's in 'prisma' folder:
```cd prisma
docker-compose up -d
prisma deploy
```
...
----
### Connect Node.js with Prisma graphql API
...
----
### Fetching schemas for given API and saving in file
Rrun: `npm install graphql-cli@2.16.4`
...
This will connect to that endpoint, fetch the schema and store it in application file /prisma.graphql. Only time when we make changes in this file is when we re-run script and fetch the latest schema. So when I make changes to datamodel.graphql I need to run `prisma deploy` and script `npm run get-schema`.
...
Add secret property to prisma.yml like `secret: my_secret_here` and the same to prisma.js but with quote `secret: 'my_secret_here'` and redeploy prisma
...
in .graphqlconfig add single property on 'extensions' obj like: `"prisma": "prisma/prisma.yml",`. Otherwise you would not be able to get schema because we locked endpoint. Then run  `npm run get-schema`
...
this is the token that server is going to send to client when client sings up or logs in. And that is the same token later user will send with any request. 
...
### Lesson 89. Setting dynamic endpoint in prisma.yml.
...
Login to prisma by runing `prisma login`, 
click "Grand permission"
...
Copy endpoint from `prisma.yml` and paste into prod.env like:
`"PRISMA_ENDPOINT=https://lp-traveller-d24b2c0c04.herokuapp.com/krystyna-lemeni-pop-traveller/prod" `

Now override prisma.yml endpoint like: 
` "endpoint: ${env:PRISMA_ENDPOINT}"`
run `prisma deploy -e ../config/prod.env` 
and you're all set with deploying to prod.
...
We can't use babel in production. That requires to make small change.
...
Run: `heroku config:set PRISMA_ENDPOINT=https://lp-traveller-d24b2c0c04.herokuapp.com/krystyna-lemeni-pop-traveller/prod
` endpoint copied from prod.env

### To see all env variables set on heroku: 
`heroku config`. These variables should match with ones in prod.env

### To push all committed code up to heroku:
`git push heroku master`, the heroku is going to install modules, run code through babel and start up app.
...
Create `test.env` file and paste dev.env content but change enpoint like:
```
...
change script in apollo `package.json` like: 
```
...
---------
### Communicate with graphql from browser? Lesson 99
...
change test in package.json like: 
```
...
### Lesson 115 !!!!! Faster way to deploy
...
```
 Try to run: 
 ```
heroku config:set PRISMA_ENDPOINT=https://us1.prisma.sh/krystyna-2fc50b/food/prod PRISMA_SECRET=thinkpositivesecret PR
PISMA_MANAGEMENT_API_SECRET=ce624c07c97ee6a -a pure-everglades-49013
```
...
(From there, try to redeploy Prisma, check the new container logs)
...
----
run `git remote -v` 
That would show you the different Heroku apps that are connected with that project.
